...
class SSDInputEncoder:
    ...
    def __call__(self, ground_truth_labels, diagnostics=False):
        ...
        # 存入gtbox坐标和分类,方便计算loss的时候arm decode后匹配用
        gt_tensor = np.zeros([y_encoded.shape[0], y_encoded.shape[1], 5])
        y_encoded = np.concatenate((y_encoded,gt_tensor), axis=2)
        for i in range(batch_size): # For each batch item...
            if ground_truth_labels[i].shape[0] == 0: continue
            for j in range(ground_truth_labels[i].shape[0]):
                y_encoded[i,j,self.n_classes+12:self.n_classes+12+5] = ground_truth_labels[i][j][0:5]
    ...
    def generate_anchor_boxes_for_layer(self,
        ...
        if self.coords == 'centroids':
            # Convert `(xmin, ymin, xmax, ymax)` back to `(cx, cy, w, h)`.(反过来cx->xmin)
