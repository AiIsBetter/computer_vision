...
def decode_detections(y_pred1,
    ...
    # 根据arm部分的预测值过滤一部分背景样本(Filter a portion of the background sample according to the predicted value of the arm section)
    a = y_pred1.shape[0]
    y_pred1 = y_pred1[y_pred1[:, :, 0] < 0.99]
    y_pred1 = y_pred1.reshape((a,y_pred1.shape[0],y_pred1.shape[1]))
    y_pred_decoded_raw = y_pred1[ :,:, 2:14]
    # arm的loc pred decode用来refine anchor(Arm's loc pred decode is used for refine anchor)
    # (0,12)格式：gt(cx, cy ,w, h) prior(cx ,cy ,w, h) variance(0.1,0.1,0.2,0.2)
    # xmin, ymin, xmax, ymax decode
    hpref = np.exp(y_pred_decoded_raw[:,:, 3:4] * y_pred_decoded_raw[:,:, -1:])
    wpref = np.exp(y_pred_decoded_raw[ :,:, 2:3] * y_pred_decoded_raw[:,:, -2:-1])  # exp(ln(w(pred)/w(anchor)) / w_variance * w_variance) == w(pred) / w(anchor), exp(ln(h(pred)/h(anchor)) / h_variance * h_variance) == h(pred) / h(anchor)
...
def decode_detections_fast(y_pred1,
