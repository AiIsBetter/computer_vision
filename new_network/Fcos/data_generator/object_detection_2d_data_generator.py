    ...
    def generate(self,
        ...
        Arguments:
            ...
            returns (set, optional): A set of strings that determines what outputs the generator yields. The generator's output
                ...
                * 'matched_anchors': Only available if `labels_encoder` is an `FCOSInputEncoder` object. The same as 'encoded_labels',
        ...
        elif not isinstance(label_encoder, FCOSInputEncoder):
            if 'matched_anchors' in returns:
                warnings.warn("`label_encoder` is not an `FCOSInputEncoder` object, therefore 'matched_anchors' is not a possible return, " +
                              "but you set `returns = {}`. The impossible returns will be `None`.".format(returns))
        ...
        while True:
            ...
            if not (label_encoder is None or self.labels is None):
                ...
                if ('matched_anchors' in returns) and isinstance(label_encoder, FCOSInputEncoder):
